...
const popularBooksDB = async () => {
    try {
        return await Books.find().sort({ bbeScore: -1 }).limit(10);
    } catch (error) {
        throw error;
    }
};
...
const popularBookGenreDB = async (genre) => {
    try {
        ...
        // const data = await Books.find({ 'genres': { '$in': [genre] } }).sort({ bbeScore: -1 }).limit(2)
...
const autoCompleteBooksDB = async (searchTerm) => {
    try {
        const data = await Books.aggregate([
            ...
            { $limit: 10 },
            { $project: { coverImg: 1, title: 1, author: 1 } },
        ]);
